...
public class horaDAO {
    public void save(Hora hora) {
        String sql = "INSERT INTO hora(cod_cr, username_lancador, cnpj_cliente, data_hora_inicio, data_hora_fim, tipo, justificativa_lancamento, projeto, username_aprovador, justificativa_negacao, status_aprovacao, solicitante_lancamento, feriado) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            pstm.setBoolean(13, hora.getFeriado());
            pstm.execute();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public Set<Hora> getHoras() {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                hora.setFeriado(rset.getBoolean("feriado"));
                horas.add(hora);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Set<Hora> getHora(String username_lancador) {
        ...
        Set<Hora> horasUser = new HashSet<>();
        Connection conn = null;
        PreparedStatement pstm = null;
        ResultSet rset = null;
    ...
    public LinkedList<TabelaAprovaçãoGestor> getHora(LinkedList<Integer> lis_int_cr) {
